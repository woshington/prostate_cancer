# %% [markdown]
# # EfficientNet Family (b0–b7) — Ordinal Focal Loss + Optuna HPO
# 
# This notebook sweeps the **entire EfficientNet family (b0 … b7)** from torchvision on the
# PANDAS ISUP-grading task. For **each** backbone it:
# 
...
#    `unfreeze_blocks`, `weight_decay`, `batch_size`, and now also `use_ordinal_loss` (focal-only vs focal+ordinal) and `ordinal_weight` (with `focal_weight = 1 - ordinal_weight`) are tuned to maximise validation QWK on a short proxy run.
...
# 
# A final cell compares all eight models and saves a summary (`logs/family-summary.{csv,txt,png}`).
# 
# > **Note on methodology.** As requested, the held-out fold (`VAL_FOLD`) is used both for Optuna
# > selection *and* for the final reported metrics. The independent `data/test.csv` set is available if
# > you later want a leakage-free estimate — just point the eval loader at it.
...
BATCH_CANDIDATES = {
    'b0': [8, 16], 'b1': [8, 16], 'b2': [8, 16], 'b3': [4, 8], 'b4': [4, 8], 'b5': [4, 8], 'b6': [4, 8], 'b7': [4, 8],
}
...
LOG_DIR    = 'logs-overlap'
MODEL_DIR  = 'models-overlap'
...
def make_loaders(batch_size):
    train_ds = PandasOverlapDataset(IMAGES_DIR, df_train, transforms=train_transforms, overlap=25)
    val_ds   = PandasOverlapDataset(IMAGES_DIR, df_val,   transforms=val_transforms,   overlap=25)
...
def objective(trial, name):
    ...
    params = dict(
        ...
        unfreeze_blocks = trial.suggest_int('unfreeze_blocks', 1, 5),
        weight_decay    = trial.suggest_float('weight_decay', 1e-6, 1e-3, log=True),
        batch_size      = trial.suggest_categorical('batch_size', BATCH_CANDIDATES[name]),
    )

    # ordinal_weight / focal_weight always sum to 1 (focal_weight = 1 - ordinal_weight).
    # Only searched when the ordinal term is enabled; fixed at 0 (pure focal) otherwise,
    # so "focal only" trials don't waste search budget on an unused knob.
    if use_ordinal_loss:
...
def run_study(name):
    ...
    study   = optuna.create_study(
        ...
        study_name=f'efficientnet-{name}-ordinal-focal',
    )
    study.optimize(
...
def save_model_artifacts(name, history, best_epoch, res, best_params, study_best):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(LOG_DIR, f'family-{name}-training.png'), dpi=200, bbox_inches='tight')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(LOG_DIR, f'family-{name}-confusion-matrix.png'), dpi=200, bbox_inches='tight')
    plt.show()

    # ── results txt ──────────────────────────────────────────────────
    with open(os.path.join(LOG_DIR, f'family-{name}-results.txt'), 'w') as f:
...
# 
# This is the heavy cell. It iterates over `MODELS_TO_RUN`, and for each backbone runs the Optuna
# search, the full training, and the validation evaluation, saving all per-model artifacts.

# %%
summary = {}

for name in MODELS_TO_RUN:
    model_path = os.path.join(MODEL_DIR, f'family-{name}.pth')

    if os.path.exists(model_path):
        print(f"EfficientNet-{name.upper()} já treinada. Pulando...")
        continue
    ...
    model_path = os.path.join(MODEL_DIR, f'family-{name}.pth')
    log_path   = os.path.join(LOG_DIR,   f'family-{name}.txt')
...
plt.tight_layout()
plt.savefig(os.path.join(LOG_DIR, 'family-comparison.png'), dpi=200, bbox_inches='tight')
plt.show()

# combined text summary + best params per model
with open(os.path.join(LOG_DIR, 'family-summary.txt'), 'w') as f:
...
print('\nSaved: logs/family-summary.csv, logs/family-summary.txt, logs/family-comparison.png')
...
with open(os.path.join(LOG_DIR, 'family-test-results.txt'), 'w') as f:
