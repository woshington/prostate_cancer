...
\begin{table*}[!t]
	\centering
	\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lllc p{5cm}@{}}
		\toprule
		\textbf{Study} & \textbf{Methodology} & \textbf{Dataset} & \textbf{Performance} & \textbf{Remarks} \\
		\midrule
		\textbf{Proposed} & \textbf{EfficientNet Ens. + Ordinal Focal} & \textbf{PANDA (10,616 WSIs)} & \textbf{QWK: 0.879} & \textbf{Focus on noise reduction (Entropy) and ordinal consistency.} \\
		\midrule
		\cite{ref-bulten2020} & Extended U-Net & Radboud (5,759 biopsies) & QWK: 0.918 & Outperformed 10 out of 15 pathologists. \\
		\cite{ref-singhal2022} & U-Net + ResNeXt50 & Multicenter (6,670 WSIs) & QWK: 0.930 & Used \textit{Active Learning} and domain adaptation. \\
		\cite{ref-nagpal2019} & InceptionV3 + k-NN & TCGA + Others (1,226) & Acc: 0.700 & Significantly outperformed the average of pathologists. \\
		\cite{ref-lucas2019} & InceptionV3 & Private (96 sections) & QWK: 0.700 & Limited dataset; high accuracy for binary classes. \\
		\cite{ref-kott2019} & ResNet-18 & Private (85 WSIs) & Acc: 85.4\% & Pilot study focused on multiclass grading. \\
		\cite{ref-hammouda2021} & Pyramidal CNN & Radboud (608 images) & Acc: 94.0\% & Small dataset, artificially balanced. \\
		\bottomrule
	\end{tabular*}
	\begin{tablenotes}
		\item Abbreviations: QWK, Quadratic Weighted Kappa; Acc, Accuracy; WSI, Whole Slide Image.
	\end{tablenotes}
	\caption{Comparison of the results obtained by the proposed method with related state-of-the-art works.\label{tab:comparison-results}}
\end{table*}
